...
integration("voting rounds", () => {
  ...
  async function member(name, group, role = "MEMBER") {
    ...
    if (group)
      await GroupMember.create({ group: group._id, user: user._id, role, status: "ACTIVE" });
    return { user, token };
  }
  ...
  async function setup() {
    ...
    for (const title of ["Portal", "Overcooked"]) {
      const game = await Game.create({
        title,
        normalizedTitle: `${title.toLowerCase()}-${++sequence}`,
        platforms: ["PC"],
        createdBy: owner.user._id,
      });
      items.push(
        await QueueItem.create({ group: group._id, game: game._id, suggestedBy: owner.user._id }),
      );
    }
  ...
  it("requires admin, valid suggestions and only one open round", async () => {
    ...
    expect(
      (await request(app).post(path).set(auth(outsider.token)).send({ candidateIds })).status,
    ).toBe(404);
    expect(
      (await request(app).post(path).set(auth(voter.token)).send({ candidateIds })).status,
    ).toBe(403);
    ...
    expect(
      (await request(app).post(path).set(auth(owner.token)).send({ candidateIds })).status,
    ).toBe(409);
    expect((await QueueItem.findById(items[0].id)).status).toBe("VOTING");
    expect(
      (
        await request(app)
          .put(`/api/v1/groups/${opened.body.data.groupId}/queue/${items[0].id}/participants`)
          .set(auth(owner.token))
          .send({ participantIds: [voter.user.id] })
      ).status,
    ).toBe(409);
  }, 15000);
  ...
  it("closes with the unique leader and resets losing suggestions for future rounds", async () => {
    ...
    const opened = await request(app)
      .post(path)
      .set(auth(owner.token))
      .send({ candidateIds: items.map((item) => item.id) });
    const roundId = opened.body.data.id;
    await request(app)
      .post(`/api/v1/groups/${group.id}/queue/${items[0].id}/votes`)
      .set(auth(voter.token))
      .send({});
    const closed = await request(app)
      .post(`${path}/${roundId}/close`)
      .set(auth(owner.token))
      .send({});
    ...
    const next = await request(app)
      .post(path)
      .set(auth(owner.token))
      .send({ candidateIds: [items[1].id] });
    expect(next.status).toBe(201);
    expect(next.body.data.candidates[0].voteCount).toBe(0);
    expect(
      (
        await request(app)
          .post(`/api/v1/groups/${group.id}/queue/${items[1].id}/votes`)
          .set(auth(voter.token))
          .send({})
      ).status,
    ).toBe(201);
    const previous = await request(app).get(path).set(auth(voter.token));
    expect(previous.body.data.rounds[1].candidates.map((candidate) => candidate.voteCount)).toEqual(
      [1, 0],
    );
  });
  ...
  it("requires an admin choice among tied leaders and preserves the tally", async () => {
    ...
    for (const item of items)
      await request(app)
        .post(`/api/v1/groups/${group.id}/queue/${item.id}/votes`)
        .set(auth(voter.token))
        .send({});
  ...
  it("cannot close without votes and can cancel without choosing a winner", async () => {
    ...
    expect(
      (await request(app).post(`${roundPath}/close`).set(auth(owner.token)).send({})).status,
    ).toBe(409);
    ...
    expect(
      (await request(app).post(`${roundPath}/cancel`).set(auth(owner.token)).send({})).status,
    ).toBe(409);
  });
});

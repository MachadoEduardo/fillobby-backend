...
describe('auth contract', () => {
  it('serializes users without password fields', () => {
    const user = { _id: '507f1f77bcf86cd799439011', name: 'Ana', email: 'ana@email.com', passwordHash: 'secret', avatarUrl: null };
    expect(serializeUser(user)).toEqual({ id: user._id, name: 'Ana', email: 'ana@email.com', avatarUrl: null });
...
integration('auth integration', () => {
  ...
  it('registers and logs in a user, then reads /me', async () => {
    const registration = await request(app).post('/api/v1/auth/register').send({ name: 'Ana Silva', email: 'ANA@EMAIL.COM', password: 'SenhaForte123' });
    expect(registration.status).toBe(201);
    expect(registration.body.data.user).toBeUndefined();
    expect(registration.body.data.email).toBe('ana@email.com');

    const login = await request(app).post('/api/v1/auth/login').send({ email: 'ana@email.com', password: 'SenhaForte123' });
    expect(login.status).toBe(200);
    expect(login.body.data.token).toEqual(expect.any(String));

    const me = await request(app).get('/api/v1/auth/me').set('Authorization', `Bearer ${login.body.data.token}`);
  ...
  it('rejects duplicate emails and invalid credentials', async () => {
    const payload = { name: 'Ana Silva', email: 'ana@email.com', password: 'SenhaForte123' };
    ...
    const invalid = await request(app).post('/api/v1/auth/login').send({ email: payload.email, password: 'Errada123' });

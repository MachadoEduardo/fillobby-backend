...
function requireAdmin(membership) {
  if (!["OWNER", "ADMIN"].includes(membership.role))
    throw new AppError("INSUFFICIENT_GROUP_ROLE", "Voce nao possui permissao para esta operacao.", 403);
}
...
async function serializeRound(round) {
  await round.populate([{ path: "startedBy", select: "name avatarUrl" }, { path: "closedBy", select: "name avatarUrl" }]);
  const liveItems = round.status === "OPEN"
    ? await QueueItem.find({ _id: { $in: round.candidates.map((candidate) => candidate.item) } }).select("voteCount")
    : [];
  ...
  return {
    ...
    startedBy: { id: round.startedBy.id, name: round.startedBy.name, avatarUrl: round.startedBy.avatarUrl ?? null },
    startedAt: round.createdAt,
    closedBy: round.closedBy ? { id: round.closedBy.id, name: round.closedBy.name, avatarUrl: round.closedBy.avatarUrl ?? null } : null,
...
export async function listRounds({ groupId, userId, page, limit }) {
  ...
  const [total, rounds] = await Promise.all([
    ...
    VotingRound.find(filter).sort({ createdAt: -1, _id: -1 }).skip((page - 1) * limit).limit(limit),
  ]);
  return {
...
export async function startRound({ groupId, userId, candidateIds }) {
  ...
  try {
    round = await transaction(async (session) => {
      ...
      if (await QueueItem.exists({ group: group._id, status: QUEUE_STATUS.VOTING, votingRound: null }).session(session))
        throw new AppError("LEGACY_VOTING_PENDING", "Encerre as votacoes antigas antes de iniciar uma rodada.", 409);

      const items = await QueueItem.find({ _id: { $in: candidateIds }, group: group._id, status: QUEUE_STATUS.SUGGESTED })
        .populate("game", "title").session(session);
      if (items.length !== candidateIds.length)
        throw new AppError("INVALID_ROUND_CANDIDATES", "Escolha apenas sugestoes disponiveis deste grupo.", 409);
      const byId = new Map(items.map((item) => [item.id, item]));
      const [created] = await VotingRound.create([{
        group: group._id,
        startedBy: userId,
        candidates: candidateIds.map((itemId) => ({ item: itemId, gameTitle: byId.get(itemId).game.title })),
      }], { session });
      ...
      if (updated.modifiedCount !== candidateIds.length)
        throw new AppError("INVALID_ROUND_CANDIDATES", "As sugestoes mudaram. Atualize a fila.", 409);
      return created;
    });
  } catch (error) {
    if (error?.code === 11000)
...
async function finishRound({ groupId, userId, roundId, winnerItemId, cancel }) {
  ...
  const round = await transaction(async (session) => {
    const current = await VotingRound.findOne({ _id: roundId, group: group._id, status: "OPEN" }).session(session);
    ...
    const items = await QueueItem.find({ _id: { $in: ids }, group: group._id, status: QUEUE_STATUS.VOTING, votingRound: current._id }).session(session);
    ...
    current.candidates.forEach((candidate) => { candidate.voteCount = counts.get(candidate.item.toString()); });

    let winnerId = null;
    if (!cancel) {
      const max = Math.max(...current.candidates.map((candidate) => candidate.voteCount));
      if (max === 0) throw new AppError("VOTING_ROUND_NO_VOTES", "Cancele a rodada sem votos.", 409);
      const leaders = current.candidates.filter((candidate) => candidate.voteCount === max).map((candidate) => candidate.item.toString());
    ...
    if (resetIds.length) {
      ...
      await QueueItem.updateMany(
        { _id: { $in: resetIds }, group: group._id, status: QUEUE_STATUS.VOTING, votingRound: current._id },
...
export function closeRound(input) { return finishRound({ ...input, cancel: false }); }
export function cancelRound(input) { return finishRound({ ...input, cancel: true }); }
